...
class SANN(nn.Module):
    def __init__(self, args, device):
        ...
        self.attention_hops_size = args.attention_hops_dim
        ...
        self.lstm = nn.LSTM(input_size=args.embedding_dim,
                            ...
                            bidirectional=False,
                            batch_first=True)
        self.W_attention_unit = torch.randn([self.hidden_size, self.attention_unit_size], requires_grad=True).to(device)
        self.W_attention_hops = torch.randn([self.attention_unit_size, self.attention_hops_size], requires_grad=True).to(device)
#        self.fc_get_attention_unit = nn.Linear(self.hidden_size, attention_unit_size)
#        self.fc_get_attention_hops = nn.Linear(attention_unit_size, attention_hops_size)
        self.fc = nn.Linear(self.batch_size * self.attention_hops_size * self.hidden_size, 2)
        self.hidden = self.init_hidden()        #初始化隐藏层
        self.memory_c = self.init_hidden()
    ...
    def forward(self, x):
        ...
        向前传播
        '''
        #out = [batch_size, pad_seq_len * 2, hidden_size]
        out, (middle_hidden, middle_memory_c) = self.lstm(x, (self.hidden, self.memory_c))
        attention_unit = torch.matmul(out, self.W_attention_unit)
        #attention_hops = [batch_size, pad_seq_len * 2, attention_hops_size]
        attention_hops = torch.matmul(F.tanh(attention_unit), self.W_attention_hops)
        print("attention_unit: {0}\tattention_hops: {1}\t".format(attention_unit.size(), attention_hops.size()))
        #M_front = [batch_size, attention_hops_size, hidden_size]
        M_front = torch.matmul(torch.transpose(attention_hops, 1, 2), out)
        #M_front_flat = [batch_size, attention_hops_size * hidden_size]
        M_front_flat = M_front.view(-1, self.attention_hops_size * self.hidden_size)
        print("M_front: {0}\tM_front_flat: {1}\t".format(M_front.size(), M_front_flat.size()))
        shaped_M_front_flat = torch.reshape(M_front_flat, (1, self.batch_size * self.attention_hops_size * self.hidden_size))
        fc_out = self.fc(shaped_M_front_flat)
